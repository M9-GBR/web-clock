...
function drawDay(date) {
    ...
    let m = ctx.measureText(dayMap[0]),
        h = m.actualBoundingBoxDescent - m.actualBoundingBoxAscent,
        maxW = 50,
        posX = clock.center - (maxW / 2),
        posY = y - h / 2

    let curM = ctx.measureText(curDay),
        curW = Math.min(curM.width, maxW),
        nextM = ctx.measureText(nextDay),
        nextW = Math.min(nextM.width, maxW)
...
function drawHands(date) {
    ...
    ctx.lineTo(0, tailLength * 2.5 - 250)
    ctx.stroke()
